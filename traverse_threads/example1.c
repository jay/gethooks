...
thread ids. First build the traverse_threads library (see BUILD.txt).

-
MinGW:

gcc -I..\include -L..\lib -o example1 example1.c -ltraverse_threads -lntdll
-

-
Visual Studio (*from Visual Studio command prompt):

cl /I..\include example1.c ..\lib\traverse_threads.lib ..\lib\ntdll.lib
-
*/
...
/* example of simple callback. print the thread id. 
...
)
{
	...
	printf( "callback says sti->ClientId.UniqueThread: %Iu\n", (size_t)sti->ClientId.UniqueThread );
